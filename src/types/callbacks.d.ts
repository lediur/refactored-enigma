...
declare namespace callbacks {
   ...
   */
  function on_always_on_top_changed(state: boolean): void;
   ...
   */
  function on_char(code: Utf16EncodedChar): void;
   ...
   */
  function on_dsp_preset_changed(): void;

  /**
   * Called when the panel gets/loses focus.
   */
  function on_focus(is_focused: boolean): void;
   ...
   * @param handle
   */
  function on_item_played(handle: FbMetadbHandle): void;
  ...
  */
  function on_key_down(vkey: FbTodo): void;
  function on_key_up(vkey: FbTodo): void;

  /**
   * Called when items are added to the library.
   * @param handle_list
   */
  function on_library_items_added(handle_list: FbMetadbHandleList): void;

  /**
   * Called when the library items change
   * @param handle_list
   */
  function on_library_items_changed(handle_list: FbMetadbHandleList): void;

  /**
   * Called when items are removed from the library
   * @param handle_list
   */
  function on_library_items_removed(handle_list: FbMetadbHandleList): void;
   ...
   * @param image
   */
  function on_load_image_done(cookie: FbTodo, image: FbTodo): void;
   ...
   * }
   */
  function on_main_menu(index: number): void;
  ...
  function on_mouse_lbtn_dblclk(x: number, y: number, mask: MouseMask): void;
  function on_mouse_lbtn_down(x: number, y: number, mask: MouseMask): void;
  function on_mouse_lbtn_up(x: number, y: number, mask: MouseMask): void;
  function on_mouse_leave(): void;
  function on_mouse_mbtn_dblclk(x: number, y: number, mask: MouseMask): void;
  function on_mouse_mbtn_down(x: number, y: number, mask: MouseMask): void;
  function on_mouse_mbtn_up(x: number, y: number, mask: MouseMask): void;
  function on_mouse_move(x: number, y: number, mask: MouseMask): void;
  function on_mouse_rbtn_dblclk(x: number, y: number, mask: MouseMask): void;
  function on_mouse_rbtn_down(x: number, y: number, mask: MouseMask): void;
  // See flags.txt > Mask for mouse callbacks

  function on_mouse_rbtn_up(x: number, y: number, mask: MouseMask): void;
   ...
   */
  function on_mouse_wheel_h(step: number): void;
   ...
   * @param info
   */
  function on_notify_data(name: FbTodo, info: FbTodo): void;

  /**
   * This callback is only available in foobar2000 v1.4 and later.
   * Called when output device changes. Use fb.GetOutputDevices to retrieve settings.
   */
  function on_output_device_changed(): void;
   ...
   * @param handle
   */
  function on_playback_edited(handle: FbMetadbHandle): void;

  /**
   * Called when a new track is selected for playback.
   * @param handle
   */
  function on_playback_new_track(handle: FbMetadbHandle): void;

  /**
   * Called when playback order is changed.
   * @param new_order_index
   */
  function on_playback_order_changed(new_order_index: PlaybackOrder): void;

  /**
   * Called when playback pauses.
   */
  function on_playback_pause(is_paused: boolean): void;
   ...
   * @param origin
   */
  function on_playback_queue_changed(origin: PlaybackQueueChangeReason): void;
  // origin:
   ...
   */
  function on_playback_seek(time: number): void;
   ...
   * @param reason
   */
  function on_playback_stop(reason: PlaybackStopReason): void;

  /**
   * Called every second a track is playing. Useful for time display.
   * @param time the elapsed fractional playback time in seconds
   */
  function on_playback_time(time: number): void;

  function on_playlist_item_ensure_visible(
    playlistIndex: number,
    playlistItemIndex: number
  ): void;

  function on_playlist_items_added(playlistIndex: number): void;
  function on_playlist_items_removed(
    playlistIndex: number,
    new_count: number
  ): void;
  function on_playlist_items_reordered(playlistIndex: number): void;

  /**
   * Workaround for some 3rd party playlist viewers not working with on_selection_changed().
   */
  function on_playlist_items_selection_change(): void;

  /**
   * Called when "stop after current" is enabled/disabled.
   * @param state the current "stop after current" state
   */
  function on_playlist_stop_after_current_changed(state: boolean): void;
   ...
   * @param new_mode
   */
  function on_replaygain_mode_changed(new_mode: ReplayGainMode): void;
